...
$name = $regNo = $email = $date = $fromTime = $toTime = $blockNumber = $roomNumber = $complaint = $complaintText = "";
$nameErr = $regNoErr = $emailErr = $dateErr = $fromTimeErr = $toTimeErr = $blockNumberErr = $roomNumberErr = $complaintErr = $complaintTextErr = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["complaint"])) {
        $complaintErr = "Complaint Type is required";
    } else {
    ...
    if (empty($_POST["complaint_text"])) {
        $complaintTextErr = "Complaint Text is required";
    } else {
        $complaintText = test_input($_POST["complaint_text"]);
    }

    // If all fields are valid, insert data into the database
    if (empty($nameErr) && empty($regNoErr) && empty($emailErr) && empty($dateErr) && empty($fromTimeErr) && empty($toTimeErr) && empty($blockNumberErr) && empty($roomNumberErr) && empty($complaintErr) && empty($complaintTextErr)) {
        ...
        $complaintText = mysqli_real_escape_string($conn, $complaintText);

        // Create the SQL query to insert data into the table
        $sql = "INSERT INTO complaint_box (name, regNo, email, date, fromTime, toTime, blockNumber, roomNumber, complaint, complaintText)
                VALUES ('$name', '$regNo', '$email', '$date', '$fromTime', '$toTime', '$blockNumber', '$roomNumber', '$complaint', '$complaintText')";

        if (mysqli_query($conn, $sql)) {
            echo "<p id='result'>Complaint form submitted successfully. Thank you!</p>";
        } else {
            ...
            <div class="form-container">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
                    ...
                    <div class="form-group horizontal select">
                        <label for="complaint">Complaint about:</label>
                        ...
                        <span class="text-danger"><?php echo $complaintErr; ?></span>
                    </div>
                    <div class="form-group horizontal">
                        <label for="complaint_text">Write your complaint here:</label>
                        <textarea name="complaint_text" id="complaint_text" class="form-control" rows="4" placeholder="Enter your complaint here"></textarea>
                        <span class="text-danger"><?php echo $complaintTextErr; ?></span>
                    </div>
